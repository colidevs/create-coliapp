 ...
 * (`code === "23505"`) — the same detection `admin/variant-option-types/
 * repository.ts` already relies on. Here the constraint is the composite
 * `uq_variant_option_values_type_slug` (`optionTypeId`, `slug`), not a
 * globally-unique column, but the driver-level error code is identical.
 */
